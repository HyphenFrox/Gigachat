...
- The workspace directory above is the conversation's root. `bash` remembers its cwd across calls — a `cd subdir` in one call carries over to the next, so you can chain work across several bash turns the same way you would in a real shell. **File tools follow that same cwd**: after you `cd stock-tracker`, `write_file({"path": "src/App.jsx", ...})` lands inside `stock-tracker/src/`, not back at the workspace root. Use absolute paths if you ever need to bypass the bash cwd and write relative to the workspace root directly.
