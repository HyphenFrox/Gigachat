...
**Adaptive `-ngl`.** Before spawning llama-server the backend reads the GGUF's `block_count` metadata, divides total file size by layer count to estimate bytes-per-layer, and computes how many layers fit in the sum of pool free memory. **Zero safety margin** — every reported free byte counts toward the budget. The result is passed as `-ngl <N>`. Layers beyond that stay on host CPU paged from the GGUF via mmap — same trick Ollama uses on the host side. Without this, the optimistic `-ngl 99` would overcommit and crash one rpc-server's allocator when memory is tight.
...
Ollama path picks up the same family of optimisations via env vars: `OLLAMA_FLASH_ATTENTION=1`, `OLLAMA_KV_CACHE_TYPE=q8_0`, `OLLAMA_NUM_PARALLEL` (hardware-tuned), `OLLAMA_NUM_THREAD` (physical cores), `OLLAMA_KEEP_ALIVE=60m`. User-set values always win so explicit overrides aren't clobbered.
