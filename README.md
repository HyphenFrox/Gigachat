...
| `--cache-reuse 256` | KV-shift-based prompt-prefix reuse across follow-up turns. When a chat shares a prefix with the previous turn (same system prompt, same first N rounds), the server skips re-decoding that prefix and shifts the cached KV state forward. Big win on multi-turn chat with stable system prompts. |
| `-b … -ub …` | Adaptive prompt-eval batch / ubatch sizes. Computed against free VRAM after weights + KV; ceiling raised to 8192 (was 4096) since FA + Q8 KV free up activation budget. Big-VRAM workstations get 2-4× faster prefill on long prompts. |
| `--mlock` | Engaged when free RAM ≥ 2× the model file. Pins weights so the OS can't page them out under memory pressure — keeps token rate from degrading on long-running hosts. |
...
Ollama path picks up the same family of optimisations via env vars: `OLLAMA_FLASH_ATTENTION=1`, `OLLAMA_KV_CACHE_TYPE=q8_0`, `OLLAMA_NUM_PARALLEL` (hardware-tuned), `OLLAMA_NUM_THREAD` (physical cores), `OLLAMA_KEEP_ALIVE=60m`, `OLLAMA_SCHED_SPREAD=1` (multi-GPU layer distribution), `OLLAMA_GPU_OVERHEAD` set to 5 % of detected VRAM. User-set values always win so explicit overrides aren't clobbered.
...
**Mega-busy host bias.** When the router commits to the mega-model path (model exceeds combined pool memory → host disk pages layers via mmap on every forward pass), the host is marked mega-busy for 5 minutes. While that flag is set, parallel work coming in afterward — subagent fan-out, embedding routing, side chats — is biased toward workers so the host's disk subsystem stays dedicated to the page-in stream rather than fighting for I/O bandwidth. `pick_chat_target` overrides its "host is more capable" rule to pick a worker; `list_subagent_workers` skips its perf-ratio gate so even slow workers join the rotation; `run_subagents_parallel` drops host from the round-robin entirely (when at least one worker is eligible). The flag refreshes on every mega-model engagement and expires automatically — a quick follow-up chat after the mega session ends goes to the strongest node again.
...
All of these are always-on. Each has its own engagement gate so it kicks in only when the pool would actually win — no flag to flip, no setting to tune. Together they shape the pool to whatever resources happen to be available: a single laptop runs everything host-only at default slot counts, while a desktop + 2 laptops fan embeddings + python_exec across workers, weight layer distribution by per-node free memory, batch concurrent chats in shared slots, and pin MoE experts off VRAM when the geometry would OOM.
...
**Distributed `web_search` dispatch.** DuckDuckGo searches via the `ddgs` Python library route to a worker that has `ddgs` installed. The probe-time `capabilities.has_ddgs` flag is the gate. Same SSH-PowerShell dispatch protocol. Worker setup is one-time — run `scripts/install-worker-tools.ps1` on each worker to `pip install ddgs` (and the read-doc parsers, see below).

**Distributed `read_doc` dispatch.** PDF / DOCX / XLSX parsing is the most CPU-heavy stateless tool in the codebase — pymupdf can chew up host CPU for seconds on a large document, fighting the inference path. The host now streams the file bytes (base64 over SSH stdin) to a worker that has the parser library installed (`pymupdf`, `python-docx`, or `openpyxl` per format). The worker parses and returns extracted text. 30 MB on-the-wire cap so multi-hundred-MB files stay local. Probe-time `capabilities.read_doc_libs` filters which workers can handle which formats — a worker missing the library gets skipped rather than dispatched to. One-time worker setup: `scripts/install-worker-tools.ps1`.

**Worker-side llama-server (speculative decoding for worker chat targets).** When the strongest single node is a worker (worker has more VRAM than host) AND the worker has `llama-server.exe` installed AND a same-family draft GGUF is already in the worker's Ollama inventory, the host SSH-spawns llama-server on the worker via `Win32_Process.Create` and routes the chat to it. Both target and draft live on the worker (no per-token RPC overhead); the host only handles HTTP. Idempotent — re-running for the same (target, draft) pair reuses the existing process via `/health` check. State is in-memory (`_WORKER_CHAT_SERVERS`) since the actual process lives on the worker; host restart re-spawns on the next chat turn. Falls back to plain worker-Ollama on any spawn failure.

**Adaptive continuous batching.** `split_lifecycle._compute_optimal_parallel` reads each model's GGUF metadata (layers, hidden size, GQA factor) to estimate KV-per-slot, then computes how many `--parallel` slots fit in the bottleneck node's free memory (host VRAM for host-only paths; min(host, every worker free RAM) for split paths). On a 24 GB workstation running an 8 B Q4 chat model, this lands at 4-8 slots — concurrent subagents and the speculative draft+verify share one warm engine instead of starting separate processes. Single-stream cost is identical to `--parallel 1` since unused slots stay quiescent. CPU-only laptops fall through to 1 slot. Ollama gets the same treatment via `OLLAMA_NUM_PARALLEL` set automatically when `ensure_running()` spawns Ollama (unless the user pre-set the env var).

**Tensor-split (`-ts`) auto-weights for heterogeneous pools.** When host VRAM and each worker's capacity differ by ≥ 1.5× — e.g. desktop with RTX 4090 + laptop with Iris Xe iGPU — `_compute_tensor_split_ratios` weights the layer distribution proportional to each device's actual free memory. Without this, equal split forces 1/N of the layers onto the smallest node, which OOMs on the iGPU before the desktop GPU is half full. Homogeneous pools are detected (same threshold) and skip `-ts` entirely so llama.cpp's built-in per-device free-memory query stays in control there.

**Tensor-parallel row dispatch (`--split-mode row`).** On Gigabit-Ethernet-class LANs (every worker probe latency ≤ 4 ms), `_should_use_row_split` engages `--split-mode row` so layer matmuls dispatch tensor-parallel across host + every RPC worker simultaneously. Faster on wired LAN than the default layer-pipeline; loses on Wi-Fi (typ. 8-30 ms RTT) where per-layer sync chatter dominates the matmul speedup. The check is conservative — Wi-Fi setups stay on layer-pipeline automatically.

**MoE expert auto-pin to CPU.** `_should_pin_experts_to_cpu` detects MoE models (Mixtral, DeepSeek-V3, Qwen 2.5 MoE) whose file size exceeds host VRAM and adds `-ot ".*_(exps|experts)\.weight=CPU"` to push expert tensors to host RAM (and via the existing `-d CPU` worker setup, every worker's RAM as well). Attention layers stay on GPU where the matmul win matters. Engages only when the geometry would OOM otherwise — small MoE models that fit GPU comfortably keep llama.cpp's default placement.

**Pool-distributed embed fan-out.** `compute_pool.embed_concurrency_limit(model)` returns 2× the number of usable embed backends (host + eligible workers, capped at 16). The four indexers in `tools.py` (`doc_index`, `_codebase_index_cwd_impl`, `_reembed_codebase_file`, the docs-URL crawler) now use `asyncio.gather` under that semaphore so every chunk's embed runs concurrently against the pool. Previously each call awaited serially, so even with N workers the indexer ran 1× — only one backend was ever busy. Net effect: a 1000-chunk codebase index parallelises to ~N× across an N-worker pool.

**Distributed `python_exec` dispatch.** Pure-compute Python snippets (no `open(`, `pathlib`, `subprocess`, etc.) automatically SSH-dispatch to a round-robin-picked worker via `dispatch_python_exec_to_worker` — the worker runs `python -I -` (isolated mode, same security profile as host) on the snippet and returns stdout. Snippets that touch the local filesystem run on host where the cwd actually exists; transport failures fall back to host silently. Frees host CPU for inference when the agent is doing one-off transforms, API checks, or stdlib computations during a chat turn.

**KV cache quantization.** `_decide_kv_precision_and_parallel` picks FP16 vs Q8 jointly with the `--parallel` slot count. When FP16 fits fewer than 4 slots due to memory pressure, switching to Q8 halves the per-slot KV cost and unlocks 2-4× more slots — strictly more throughput on concurrent workloads at < 1% accuracy loss. Pools with plenty of headroom stay on FP16 (no precision penalty when there's no bottleneck).

**Adaptive context window.** `_compute_optimal_ctx_size` reads the GGUF's max context, the chosen `--parallel` count, and the bottleneck-node free memory, then picks the largest `-c <N>` the pool can hold without OOM. Floors at 4 K (legacy default) and ceils at the model's training-time max. Rich pools (host+workers with combined 80 GB after target) get 32 K-128 K context windows automatically; tight pools stay at 4 K.

**Prompt-cache-to-disk reload.** `llama-server` is spawned with `--prompt-cache <path> --prompt-cache-all` keyed by GGUF + KV precision (`~/.gigachat/llama-cpp/prompt-cache/<hash>.bin`). After the first turn writes the cache, every subsequent spawn for that (model, precision) pair reloads the decoded KV from disk — system-prompt + tool-schema prefix is hot from the first token instead of cold. Big TTFT win after a model switch.

**Per-conversation worker affinity.** `pick_chat_target(model, conv_id=...)` consults `_CONV_AFFINITY` so a chat that landed on worker-A on its previous turn keeps landing there as long as A is eligible — KV cache stays warm across follow-up turns. Concurrent conversations naturally spread across the pool because the picker breaks ties on `_ACTIVE_TURNS_PER_NODE` count: turn-2 of conversation B prefers a less-loaded node when the strongest is already serving conversation A. `register_turn_start` / `register_turn_end` from `agent.run_turn` keep the active-turn ledger accurate.

**Pool-distributed compaction.** Auto-compaction summaries route through `pick_compaction_target(model)` so a worker with the model installed runs the summary while the host's KV cache stays warm for the active chat. Round-robins across eligible workers; falls back to host on single-node pools.

**Background idle re-indexing.** Every 5-minute probe sweep, `_drain_idle_reindex` walks every `status=ready` codebase index, finds files modified since `last_indexed_at`, and re-embeds them via the pool's distributed embed fan-out. Skipped when any chat turn is active (so it never competes for embed bandwidth). Capped at 20 files per sweep to bound the cost. Catches external edits (IDE writes, git checkouts) that bypass the inline `write_file` invalidation.

**Synchronous draft SCP for speculative.** `await_draft_for(target)` extends `pick_draft_for` with a blocking worker→host SCP: when no host-resident draft exists but a worker has a same-family candidate < 2 GB, the chat blocks ~20-40 s on Gigabit-Ethernet to pull it, then engages speculative decoding for the rest of the turn (and every subsequent turn). Failed pulls fall through to plain Ollama silently. The size cap keeps the wait inside one turn's tolerance.

**Per-node disk + cache visibility.** The SSH spec probe now reports `disk_total_gb`, `disk_free_gb`, and `cached_overrides` (list of cached GGUF override filenames) per worker. `pool_inventory_summary` exposes a `nodes[]` array with per-node RAM / VRAM / disk / cache state so the user can see at a glance which nodes have spare storage to absorb new model pulls.
